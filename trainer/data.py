...
class BioASQPairwiseIterator(BioASQPointwiseIterator):
    ...
    def __next__(self):
        ...
        pos_inputs = self.tokenizer(
            q_text, doc_pos_text, truncation=True, max_length=self.max_length
        )
        neg_inputs = self.tokenizer(
            q_text, doc_neg_text, truncation=True, max_length=self.max_length
        )
        self.index += 1
...
class RankingIterator(BioASQPointwiseIterator):
    ...
    def get_n_samples(dataset):
        """
        Returns the total number of positive and negative documents in the dataset.
        """
        return (sum([len(x["pos_docs"]) for x in dataset.values()]) +
                sum([len(x["neg_docs"]) for x in dataset.values()]))
...
# 3. Tests the trained model with a new set of queries
class InferenceDataset(torch.utils.data.IterableDataset):
    def __init__(
        ...
        with open(bm25_run_path) as f:
            dataset = {
                q_data["id"]: {
                    "documents": q_data["documents"][:at],  # Limit the number of considered documents
        ...
        if gs_path is not None:
            qrels = get_qrels(gs_path)  # Relevant documents to the query are mapped with a "1".
...
# 1. Create a dataset class that wraps the data
def create_training_dataset(positive_data_path, negative_data_path, collection_data_path, tokenizer, **kwargs):
    _dataset = {}
    _qrels_dict = {}
    # Positive = Relevant
    with open(positive_data_path) as f:
        for line in f:
            ...
            _dataset[sample["id"]] = {
                "pos_docs": sample["documents"],
                "question": sample["body"],
            }
            # Relevant documents to the query are mapped with a "1"
            _qrels_dict[sample["id"]] = {docid: 1 for docid in sample["documents"]}

    q_to_remove = []

    # Negative = Non-relevant
    with open(negative_data_path) as f:
        for line in f:
            sample = json.loads(line)
            _dataset[sample["id"]]["neg_docs"] = sample["neg_docs"]
            if len(sample["neg_docs"]) == 0:  # Remove queries without negative documents
    ...
    """
    _dataset is a dictionary with the following structure.
    The queries without negative documents are removed from the dataset.
    
    _dataset -> {
      "id": {
        "positive_documents":[{"id": "...", "text": "..."}], 
        "negative_documents": [{"id": "...", "text": "..."}],
        "query":"..."
      }
    }
    
    _qrels is a dictionary that contains the relevant documents to the query.
    If the document is relevant, it is mapped with a "1"., else it is not in the dictionary.
    ...
    """
    _collection is a dictionary that contains every document, mapped by its id.
...
def create_bioASQ_datasets(positive_data_path, negative_data_path, tokenizer, subsets=["train", "test"],
                           test_split_percentage: float = 0.05, **kwargs):
...
# 2. Create an iterable dataset for training
class BioASQDataset(torch.utils.data.IterableDataset):
    def __init__(
            ...
            qrels_dict=None,
            collection=None,
            max_length=-1,  # Tokenizer
            max_questions: int = -1,  # Queries
            max_neg_docs: int = -1,  # Non-relevant documents
    ):
        """
        dataset: {query_id: {}}
        """
    ...
    def __iter__(self):
        """
        We use one iterator per epoch.
        During the epoch, the iterator is called multiple times.
        """
        self.epoch += 1
        ...
        if worker_info is None:  # single-process data loading, return the full iterator
            ...
            _dataset = {worker_id: self.dataset[worker_id] for worker_id in this_worker_ids}
