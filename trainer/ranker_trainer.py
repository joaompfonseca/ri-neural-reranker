...
from transformers.trainer_pt_utils import IterableDatasetShard, nested_truncate, nested_numpify, nested_concat, \
    find_batch_size
from transformers.trainer_utils import seed_worker, EvalLoopOutput, has_length, EvalPrediction, denumpify_detensorize, \
...
class RankerTrainer(Trainer):
    ...
    def prediction_step(
        ...
        # print("here")
        _, how, _ = super().prediction_step(model, inputs["inputs"], prediction_loss_only, ignore_keys)
    ...
    def evaluation_loop(
        ...
        for step, inputs in enumerate(dataloader):
            ...
            # Prediction step
            loss, logits, labels, metadata = self.prediction_step(model, inputs, prediction_loss_only,
        ...
        if self.compute_metrics is not None and all_preds is not None:
            if args.include_inputs_for_metrics:
                metrics = self.compute_metrics(
                    RankingEvalPrediction(predictions=all_preds, label_ids=all_labels, inputs=all_inputs,
                                          ranking_metadata=ranking_metadata)
                )
            else:
                metrics = self.compute_metrics(RankingEvalPrediction(predictions=all_preds, label_ids=all_labels,
                                                                     ranking_metadata=ranking_metadata))
        else:
            metrics = {}
        ...
        if metrics:
            ...
            return RankingEvalLoopOutput(predictions=all_preds, label_ids=all_labels, metrics=metrics,
                                         num_samples=num_samples, ranking_metadata=ranking_metadata)
    ...
    def predict(
        ...
        return RankingPredictionOutput(predictions=output.predictions, label_ids=output.label_ids,
                                       metrics=output.metrics, ranking_metadata=output.ranking_metadata)
